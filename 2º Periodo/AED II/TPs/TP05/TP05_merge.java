...
public class TP05_heap {
    ...
    static class Game {
        ...
        Game(String[] dados) {
            ...
            // Mantém a data como está vindo do CSV (ex: "Oct 8, 2010" ou "Jul 2020")
    ...
    private static boolean maior(Game a, Game b) {
        numeroComparacoes++;
        String[] propAParts = a.proprietarios.split(" ");
        String[] propBParts = b.proprietarios.split(" ");
    ...
    private static void heap2(Game[] array, int n, int i) {
        ...
        if (esq < n && maior(array[esq], array[maiorIdx])) maiorIdx = esq;
        if (dir < n && maior(array[dir], array[maiorIdx])) maiorIdx = dir;
        if (maiorIdx != i) {
        ...
        for (int i = 0; i < totalDeGames; i++) {
            ...
            if (dados[0] != null && dados[0].length() > 0) {
                for (int j = 0; j < dados.length; j++) {
                    if (dados[j] != null) dados[j] = removEspaco(dados[j]);
                }
        ...
        for (int i = 0; i < selectedGamesCount; i++) {
            ...
            String dataFormatada = formatarDataParaSaida(g.lancamento);
             System.out.printf(java.util.Locale.US, "=> %d ## %s ## %s ## %s ## %.2f ## %s ## %d ## %.1f ## %d ## %s ## %s ## %s ## %s ## %s ##\n",
                    g.id, g.nome, dataFormatada, g.proprietarios, g.preco, g.idiomas,
                    g.metacritic, g.nota, g.conquistas, g.publishers, g.developers,
                    g.categorias, g.generos, g.tags);
        }
        double tempoExecucaoSort = (endTimeSort - startTimeSort) / 1000.0;
        //ARQUIVO DE LOG(pesquisei para fazer pois não sabia como fazer)
